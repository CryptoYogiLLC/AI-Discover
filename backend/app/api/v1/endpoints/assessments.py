...
from app.schemas.assessment import (
    ApplicationAssessmentCreate, ApplicationAssessmentUpdate, 
    ApplicationAssessmentResponse, AssessmentListResponse,
    AssessmentSummary
)
...
async def get_assessment_or_404(
    ...
    result = await db.execute(
        select(ApplicationAssessment).options(
...
async def list_assessments(
    ...
    if project_id:
        ...
        if current_user.role != UserRole.ADMIN:
            ...
            project_query = select(Project.id).join(ProjectMember).where(
    ...
    query = query.offset((page - 1) * size).limit(size).order_by(
        ApplicationAssessment.assessment_date.desc()
    )
    ...
    for assessment in assessments:
        response = ApplicationAssessmentResponse(
            **assessment.__dict__,
            ...
            assessed_by_username=assessment.assessed_by_user.username if assessment.assessed_by_user else None
        )
...
@router.post("/", response_model=ApplicationAssessmentResponse, status_code=status.HTTP_201_CREATED)
...
async def get_assessment_summary(
    ...
    for assessment in assessments:
        ...
        # Cost savings
        if assessment.expected_cost_savings_percent and assessment.current_licensing_cost_annual:
            savings = (assessment.expected_cost_savings_percent / 100) * assessment.current_licensing_cost_annual

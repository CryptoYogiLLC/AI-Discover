...
async def list_projects(
    ...
    for project in projects:
        response = ProjectResponse(
            **project.__dict__,
            created_by_username=project.created_by_user.username if project.created_by_user else None,
...
async def create_project(
    ...
    logger.info("Project created", project_id=str(project.id), user_id=str(current_user.id))
...
async def get_project(
    ...
    return ProjectResponse(
        **project.__dict__,
        created_by_username=project.created_by_user.username if project.created_by_user else None,
        member_count=len(project.members),
        assessment_count=len(project.assessments) if hasattr(project, 'assessments') else 0,
...
async def update_project(
    ...
    logger.info("Project updated", project_id=str(project_id), user_id=str(current_user.id))
...
async def delete_project(
    ...
    logger.info("Project deleted", project_id=str(project_id), user_id=str(current_user.id))
...
async def add_project_member(
    ...
    existing_member = next(
        (m for m in project.members if m.user_id == member_data.user_id),
        None
    )
...
async def remove_project_member(
    ...
    project = await get_project_or_404(project_id, db, current_user, required_permission)

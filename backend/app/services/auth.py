...
class AuthService:
    ...
    async def create_user(self, user_create: UserCreate, is_ldap_user: bool = False) -> User:
        """
        Create new user
        ...
        user = User(
            ...
            hashed_password=get_password_hash(user_create.password) if not is_ldap_user else None
        )
    ...
    async def get_user_by_username_or_email(self, username: str, email: Optional[str] = None) -> Optional[User]:
    ...
    async def refresh_user_role(self, user_id: str) -> Optional[UserRole]:
        ...
        if new_role and new_role != user.role:
            ...
            logger.info("User role updated from LDAP", user_id=user_id, new_role=new_role.value)
            return new_role
    ...
    async def _sync_ldap_user(self, ldap_info: Dict[str, Any]) -> Optional[User]:
        ...
        try:
            ...
            if user:
                ...
                # Create new user
                user = User(
                    email=ldap_info.get('email', f"{ldap_info['username']}@example.com"),
            ...
            logger.info("LDAP user synced", user_id=str(user.id), username=user.username)
            return user

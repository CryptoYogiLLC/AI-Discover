...
class LDAPService:
    ...
    def __init__(self):
        ...
        self.user_search_filter = settings.LDAP_USER_SEARCH_FILTER or "(sAMAccountName={username})"
        self.group_search_filter = settings.LDAP_GROUP_SEARCH_FILTER or "(member={user_dn})"
    ...
    def _authenticate_user_sync(self, username: str, password: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            if self.auth_type == "NTLM":
                user_conn = Connection(
                    self.server,
                    user=f"{settings.LDAP_DOMAIN}\\{username}" if settings.LDAP_DOMAIN else username,
                    password=password,
                    authentication=NTLM,
                    auto_bind=True
                )
            ...
            logger.info("LDAP authentication successful", username=username, role=user_info['role'])
            return user_info
            
        except LDAPException as e:
            logger.error("LDAP exception during authentication", error=str(e), username=username)
            return None
        finally:
    ...
    def _extract_user_info(self, ldap_entry) -> Dict[str, Any]:
        ...
        return {
            'username': get_attr(ldap_entry, 'sAMAccountName') or get_attr(ldap_entry, 'uid'),
            'email': get_attr(ldap_entry, 'mail') or get_attr(ldap_entry, 'userPrincipalName'),
            'full_name': get_attr(ldap_entry, 'displayName') or get_attr(ldap_entry, 'cn'),
    ...
    def _determine_user_role(self, groups: List[str]) -> UserRole:
        ...
        for group_name, role in sorted(
            ...
            key=lambda x: [UserRole.ADMIN, UserRole.COLLABORATOR, UserRole.VIEWER].index(x[1])
        ):

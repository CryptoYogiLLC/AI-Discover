...
class ApplicationAssessmentBase(BaseModel):
    ...
    documentation_quality: Optional[str] = Field(
        None, pattern="^(Excellent|Good|Fair|Poor)$"
    )
    ...
    business_impact_if_failed: Optional[str] = Field(
        None, pattern="^(Critical|High|Medium|Low)$"
    )
    rollback_complexity: Optional[str] = Field(
        None, pattern="^(Simple|Moderate|Complex)$"
    )
